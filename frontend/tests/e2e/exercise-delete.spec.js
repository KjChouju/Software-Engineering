const { test, expect } = require('@playwright/test');

test('TC-FE-EX-002 删除后统计一致', async ({ page }) => {
  await page.addInitScript(() => localStorage.clear());

  await page.route('**/api/user/login', route => {
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ code: 0, data: { id: 1, userName: '测试用户', userRole: 'user' } })
    });
  });

  let deleted = false;
  await page.route('**/api/fitness/exercise/my/list/page', route => {
    const data = deleted
      ? { code: 0, data: { records: [], total: 0, current: 1, pageSize: 100 } }
      : {
          code: 0,
          data: {
            records: [
              { id: 1, exerciseType: '跑步', duration: 30, caloriesBurned: 300, dateRecorded: new Date().toISOString().split('T')[0] }
            ],
            total: 1,
            current: 1,
            pageSize: 100
          }
        };
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(data) });
  });

  await page.route('**/api/fitness/exercise/delete', route => {
    deleted = true;
    route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ code: 0 }) });
  });

  await page.goto('/login');
  await page.getByPlaceholder('请输入您的账号').fill('user1');
  await page.getByPlaceholder('请输入您的密码').fill('password123');
  await page.getByRole('button', { name: '登录' }).click();
  await expect(page).toHaveURL('http://localhost:8080/');

  await page.getByRole('link', { name: '健身数据', exact: true }).first().click();
  await expect(page).toHaveURL('http://localhost:8080/data');

  await expect(page.locator('.workout-item')).toHaveCount(1);

  const actions = page.locator('.workout-item .workout-actions').first();
  await actions.locator('.arco-btn').last().click();

  await expect(page.locator('.workout-item')).toHaveCount(0);

  const stats = page.locator('.workout-stats .stat-card .stat-number');
  await expect(stats.nth(0)).toHaveText('0');
  await expect(stats.nth(1)).toHaveText('0');
  await expect(stats.nth(2)).toHaveText('0');
  await expect(stats.nth(3)).toHaveText('0');
});

